...
func testRescan(harness *neutrinoHarness, t *testing.T) {
	...
	spendReport, err := harness.svc.GetUtxo(
		...
		spv.StartBlock(&headerfs.BlockStamp{Height: 1101}),
	)
	if err != nil {
...
func testStartRescan(harness *neutrinoHarness, t *testing.T) {
	...
	spendReport, err := harness.svc.GetUtxo(
		...
		spv.StartBlock(&headerfs.BlockStamp{Height: 801}),
	)
	if err != nil {
		t.Fatalf("Couldn't get UTXO %s: %s", ourOutPoint, err)
	}
...
func startRescan(t *testing.T, svc *spv.ChainService, addr btcutil.Address,
	startBlock *headerfs.BlockStamp, quit <-chan struct{}) (
	*spv.Rescan, <-chan error) {

	rescan := spv.NewRescan(
		&spv.RescanChainSource{svc},
		spv.QuitChan(quit),
		spv.WatchAddrs(addr),
...
func banPeer(t *testing.T, svc *spv.ChainService, harness *rpctest.Harness) {
	t.Helper()
