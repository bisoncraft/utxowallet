...
type Config struct {
	// Chain is the blockchain that the peer is communicating on.
...
func (p *Peer) writeMessage(msg wire.Message, enc wire.MessageEncoding) error {
	...
	log.Tracef("%v", newLogClosure(func() string {
		...
		_, err := bisonwire.WriteMessageWithEncodingN(&buf, msg, p.ProtocolVersion(),
			p.cfg.Net, enc)
		if err != nil {
			return err.Error()
		}
	...
	n, err := bisonwire.WriteMessageWithEncodingN(p.conn, msg,
		p.ProtocolVersion(), p.cfg.Net, enc)
